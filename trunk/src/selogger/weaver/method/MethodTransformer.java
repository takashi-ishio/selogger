...
public class MethodTransformer extends LocalVariablesSorter {
	...
	public void visitCode() {
		...
		if (config.recordExecution() || config.recordLabel()) {
			lastDataId = newLocal(Type.INT_TYPE);
			super.visitLdcInsn(0);
			generateNewVarInsn(Opcodes.ISTORE, lastDataId);
		}
	...
	public void visitLabel(Label label) {
		...
		if (config.recordLabel()) {
			generateNewVarInsn(Opcodes.ILOAD, lastDataId);
	...
	public void visitInsn(int opcode) {
		...
		if (OpcodesUtil.isReturn(opcode)) {
			...
			if (config.recordExecution() || config.recordLabel()) {
				int dataId = generateLoggingPreservingStackTop(EventType.THROW, Descriptor.Object, "");
				generateLocationUpdate(dataId);
			}
			super.visitInsn(opcode);
...
//				// record return value
//				String returnDesc = getReturnValueDesc(desc);
//				generateLoggingPreservingStackTop(EventType.CALL_RETURN, Descriptor.Object, "CallParent=" + firstDataId);
//			} else {
				// Call an occurrence of a call
				int callId = generateLogging(EventType.INVOKE_DYNAMIC, Descriptor.Void, label);

				// Call the original method
				super.visitInvokeDynamicInsn(name, desc, bsm, bsmArgs);
				
				generateLoggingPreservingStackTop(EventType.INVOKE_DYNAMIC_RETURN, Descriptor.Object, "CallParent=" + callId);
//			}
		} else {
			super.visitInvokeDynamicInsn(name, desc, bsm, bsmArgs);
		}
