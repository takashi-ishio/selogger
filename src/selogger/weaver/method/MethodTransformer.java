...
public class MethodTransformer extends LocalVariablesSorter {
	...
	public void visitTypeInsn(int opcode, String type) {
		if (opcode == Opcodes.NEW) {
			...
			if (config.recordMethodCall() && config.recordParameters()) {
				generateLogging(EventType.NEW_OBJECT, Descriptor.Void, InstructionAttributes.of(ATTRIBUTE_OBJECT_TYPE, OpcodesUtil.getReadableTypeName(type)));
			...
			if (config.recordArrayInstructions()) {
				generateLoggingPreservingStackTop(EventType.NEW_ARRAY, Descriptor.Integer, InstructionAttributes.of(ATTRIBUTE_OBJECT_TYPE, OpcodesUtil.getReadableTypeName(type)));
			...
			if (config.recordObject()) {
				generateLoggingPreservingStackTop(EventType.OBJECT_INSTANCEOF, Descriptor.Object, InstructionAttributes.of(ATTRIBUTE_OBJECT_TYPE, OpcodesUtil.getReadableTypeName(type)));
	...
	public void visitMethodInsn(int opcode, String owner, String name, String desc, boolean itf) {
		...
		if (config.recordMethodCall()) {
			...
			InstructionAttributes attr = InstructionAttributes.of(ATTRIBUTE_OPCODE, OpcodesUtil.getString(opcode))
				.and(ATTRIBUTE_OWNER, OpcodesUtil.getReadableTypeName(owner))
